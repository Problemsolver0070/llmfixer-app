...
export function DiscountCodesList({ refreshTick = 0 }: Props) {
  ...
  const filters: DiscountCodesListFilters = {
    status,
    batch_id: batchId,
    search,
    limit: PAGE_SIZE,
  ...
  const { rows, total, loading, error, refresh } = useDiscountCodesList(filters);
...
function CodeRow({
  ...
  return (
    <tr>
      ...
      <td style={td}>{code.discount_pct}%</td>
      <td style={td}>
        {code.used_count} / {code.max_uses}
      </td>
      <td style={td}>
        <AppliesToCell skus={code.applies_to_plan_skus} />
      </td>
      <td style={td}>
        {code.expires_at
...
function AppliesToCell({ skus }: { skus: string[] }) {
